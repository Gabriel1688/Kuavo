 ...
 * @brief Whole-body controller test — multi-source aware
 *
...
public:
    bool init(uint32_t num_joints, uint32_t control_freq) {
        ...
        layout_->version = 2;  // v2 = multi-source
        ...
        printf("Controller initialized: %u joints @ %u Hz (multi-source v2)\n",
               num_joints, control_freq);
        printf("SHM layout size: %zu bytes\n", sizeof(SharedMemoryLayout));
        printf("  ImuStageData:        %zu bytes (alignas 64)\n", sizeof(ImuStageData));
        printf("  MotorGroupStageData: %zu bytes (alignas 64)\n", sizeof(MotorGroupStageData));
    ...
    void run(double duration_seconds) {
        ...
        while (g_running && iteration < total_iterations) {
            ...
            // ---- Round-trip latency (cmd → compose timestamp) ----
            if (sensor.compose_timestamp_ns > 0 && cmd.timestamp_ns > 0 &&
                sensor.compose_timestamp_ns > cmd.timestamp_ns) {
                // Not a true round-trip — measures compose freshness relative to command
            }
        ...
        uint64_t total_elapsed = get_monotonic_ns() - (get_monotonic_ns() - iteration * loop_period_ns_);

        printf("\n");
        printf("============================================================\n");
        printf("  CONTROLLER TIMING REPORT (Multi-Source v2)\n");
        printf("============================================================\n");
