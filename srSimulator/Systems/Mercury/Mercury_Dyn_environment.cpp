...
Mercury_Dyn_environment::Mercury_Dyn_environment() : num_substep_rendering_(15),
                                                     count_(0) {
    _ParamterSetup();
    ParamHandler handler("/home/gabriel_wang/work/Kuavo/config/ESTIMATOR_simple_average.yaml");
    double xdot_limit(0.1);
    double ydot_limit(1.0);
    double t_const(1.5);

    handler.getValue("t_const", t_const);
    handler.getValue("xdot_limit", xdot_limit);
    handler.getValue("ydot_limit", ydot_limit);

    m_Mercury = new Mercury(Vec3(0.0, 0.0, 0.0), srSystem::FIXED, srJoint::TORQUE);
    m_Space = new srSpace();
    m_ground = new Ground();

    interface_ = new Mercury_interface(xdot_limit, ydot_limit, t_const);
    ...
    cur_imu_acc.clear();
    for (int i = 0; i < 3; i++) {
        prev_imu_pos.push_back(0.0);
        cur_imu_pos.push_back(0.0);
        prev_imu_vel.push_back(0.0);
        cur_imu_vel.push_back(0.0);
        cur_imu_acc.push_back(0.0);
    }
}
// How often this call back is called?
void Mercury_Dyn_environment::ContolFunction(void *_data) {
    ...
    //TODO:: rfoot_contact, lfoot_contact will get from the gamepad
    bool rfoot_contact(false);
    bool lfoot_contact(false);
    // Doesn't apply the value of torque command?
    std::vector<double> torque_command(robot->num_act_joint_, 0.);
    //what's the relation between imu and LED position ?
    ...
    // PVT control command for motor/joint,
    // 1) what's the difference between pvt of motor and joint ?
    // 2) why motor and joint share the same torque?
    // 3) where get the robot->joint_[i]->m_State.m_rValue[3]?
    // 4) how to implement this in the Mujoco environment?
    int lj_start_idx(4);
    // Right
    for (int i(0); i < 3; ++i) {
    ...
    // 1) how to pass command within in the same process?
    // 2) Do we still need the dyn_env ?
    ...
    // 1) Why does hard code the kp, kd and k_friction, where to get the value in the real target ?
    // 2) what's the meaning of k_friction, how to calculate it?
    double Kp(300.0);
    double Kd(50.0);
    ...
    // Right
    for (int i(0); i < 3; ++i) {
        robot->r_joint_[i]->m_State.m_rCommand = pDyn_env->cmd_->jtorque_cmd[i] + Kp * (pDyn_env->cmd_->jpos_cmd[i] - p_data->joint_jpos[i]) + Kd * (pDyn_env->cmd_->jvel_cmd[i] - p_data->motor_jvel[i]) + K_friction * (0. - p_data->motor_jvel[i]);
    }
    // Left
    for (int i(0); i < 3; ++i) {
        robot->r_joint_[i + lj_start_idx]->m_State.m_rCommand =
            pDyn_env->cmd_->jtorque_cmd[i + 3] + Kp * (pDyn_env->cmd_->jpos_cmd[i + 3] - p_data->joint_jpos[i + 3]) + Kd * (pDyn_env->cmd_->jvel_cmd[i + 3] - p_data->motor_jvel[i + 3]) + K_friction * (0. - p_data->motor_jvel[i + 3]);
    }
...
void Mercury_Dyn_environment::PushRobotBody() {
    ...
    if (push_time_.size() > push_idx) {
        // why does below check for servo_rate, where does this value is updated/set?
        if (count_ * mercury::servo_rate > push_time_[push_idx]) {
            int body_lk_idx(m_Mercury->link_idx_map_.find("body")->second);
            // Force Setting -> value and direction
            ...
            //what's the parameters of ext_force?
            //how to get this force in the real-target?
            //why this force is added only to the body?
            //how to apply it in the Mujoco?
...
void Mercury_Dyn_environment::PassiveAnkleSpring() {
    ...
    double des_pos(-0.5);
    //what's the purpose of this function?
    //how to set it in the Kuavo which has no spring mechanism?
...
//what's the purpose of this function, difference between this and FixXY?
...
//Do we still need this function in the Mujoco?
...
void Mercury_Dyn_environment::_ParamterSetup() {
    ParamHandler handler("/home/gabriel_wang/work/Kuavo/config/SIM_sr_sim_setting.yaml");
...
void Mercury_Dyn_environment::getFootContact_Data(bool &left_foot_contact,
    ...
    //what's the meaning of lfoot_frame(2,3)?
...
void Mercury_Dyn_environment::getIMU_Data(std::vector<double> &imu_acc,
    ...
    Eigen::Matrix3d Rot;
    Rot << imu_frame(0, 0), imu_frame(0, 1), imu_frame(0, 2),
    ...
    // how to set this in the Mujoco?
    // how to handle the value when get from the real imu?
    for (int i(0); i < 3; ++i) {
        imu_ang_vel[i] = imu_se3_vel[i] + imu_ang_vel_bias_[i] + dynacore::generator_white_noise(0., imu_ang_vel_var_[i]);
    ...
    //what's the bias value for the real imu
    static double pos_x_bias = imu_frame(0, 3);
    static double pos_y_bias = imu_frame(1, 3);
    static double pos_z_bias = imu_frame(2, 3);

    std::vector<double> pos_bias;
    pos_bias.push_back(pos_x_bias);
    pos_bias.push_back(pos_y_bias);
    pos_bias.push_back(pos_z_bias);
